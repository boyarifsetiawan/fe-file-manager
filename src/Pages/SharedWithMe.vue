...
<script setup>
	...
	const allFiles = ref({
		data: fileStore.fileSharedWithMe?.data ?? [],
		next: fileStore.fileSharedWithMe.links?.next ?? null,
	});
	...
	onUpdated(() => {
		allFiles.value = {
			data: fileStore.fileSharedWithMe.data,
			next: fileStore.fileSharedWithMe?.links?.next,
		};
	});

	onMounted(() => {
		fileStore.fetchFileSharedWithMe()
